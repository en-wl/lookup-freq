...
int main() {
  {
    ...
    WordLookupByStr word_lookup{word_buffer};
    ...
    word_lookup.save("WordLookup.dat");
  }
  printf("Sorting...\n");
  auto merger = [](auto & x, const auto & y) {x.freq += y.freq;};
  {
    MutTable<Freqs> tmp;
    merge(tmp, [](auto row){return make_tuple(row.wid, row.year);}, merger);
  } {
    MutTable<FreqsPos> tmp;
    merge(tmp, [](auto row){return make_tuple(row.wid, row.year, row.pos.id);}, merger);
  } {
    MutTable<PosTotals> tmp;
    merge(tmp, [](auto row){return make_tuple(row.year, row.pos.id);}, merger);
  } {
    MutTable<Totals> tmp;
    merge(tmp, [](auto row){return row.year;}, merger);
  }
}
